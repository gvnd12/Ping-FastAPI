...
class UserOP:
    ...
    async def delete_user(self):
        ...
        neo4j_result = await Neo4jDB(
            parameters={"_id": self.document["user_id"]},
    ...
    async def change_password(self):
        ...
        verification = await verify_password(plain_pass=old_password, hashed_pass=db_pass)

        if verification:
            if self.document["new_password"] == self.document["confirm_password"]:
                new_password = await password_hash(password=self.document["new_password"])
                result = await MongoDB(
                    database=settings.USER_IDENTITY,
                    collection_name=settings.USERS_LIST,
                    filter_param={
